...
Set an optional ```json l ``` tag whenever metadata changes (and on the first event of each dataset). This allows to retrieve a smaller set of events for each dataset to filter for a specific dataset hash.
...
```json
{
  ...
  "tags":
    ...
    /* optional: */
    ["l", "<set this tag only when changing metadata fields, see section about filtering datasets>"]
    /*
